...
const UA = "OneSmarter-Visibility-Scanner/1.0 (+https://onesmarter.com/practices)";
...
function normalizeUrl(value) {
  if (typeof value !== "string" || !value.trim()) throw new AppError("Please provide a website address.");
  let input = value.trim();
  if (!/^https?:\/\//i.test(input)) input = `https://${input}`;
  let url;
  try { url = new URL(input); } catch { throw new AppError("That website address is not valid."); }
  if (!["http:", "https:"].includes(url.protocol)) throw new AppError("Only HTTP and HTTPS addresses are supported.");
  if (url.username || url.password) throw new AppError("Website addresses containing credentials are not allowed.");
  url.hash = "";
  return url;
}
...
async function assertPublic(url) {
  ...
  if (net.isIP(host)) {
    if (privateAddress(host)) throw new AppError("Private or local network addresses cannot be scanned.");
    return;
  }
  let records;
  try { records = await dns.lookup(host, { all: true, verbatim: true }); }
  catch { throw new AppError("The website hostname could not be resolved.", 422); }
  ...
  for (let redirects = 0; redirects <= 5; redirects += 1) {
    ...
    try {
      response = await fetch(url, {
        ...
        headers: { "user-agent": UA, accept: options.accept || "text/html,application/xhtml+xml,*/*;q=.2" }
      });
    } catch (error) {
      if (error?.name === "AbortError") throw new AppError("The website took too long to respond.", 504);
    ...
    if (declared > maxBytes) throw new AppError("The webpage is too large to scan safely.", 413);
    const text = await response.text();
    if (Buffer.byteLength(text) > maxBytes) throw new AppError("The webpage is too large to scan safely.", 413);
...
function textOnly(value = "") { return decode(value.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim()); }
...
async function optionalFile(origin, path) {
  ...
  try {
    const { response, text, finalUrl } = await fetchPublic(url, { maxBytes: 400000, timeout: 7000, accept: "text/plain,*/*;q=.2" });
    const exists = response.ok && text.trim() && !/<html\b/i.test(text.slice(0, 400));
    return { exists: Boolean(exists), text: exists ? text : "", url: finalUrl.toString(), bytes: Buffer.byteLength(text) };
  } catch { return { exists: false, text: "", url: url.toString(), bytes: 0 }; }
}
...
function robotsResult(text) {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    } else if ((key === "allow" || key === "disallow") && group) group.rules.push({ key, value });
  }
  const blocked = BOTS.filter((bot) => {
    const relevant = groups.filter((g) => g.agents.includes(bot.toLowerCase()) || g.agents.includes("*"));
    const deny = relevant.some((g) => g.rules.some((r) => r.key === "disallow" && r.value === "/"));
    const allow = relevant.some((g) => g.rules.some((r) => r.key === "allow" && r.value === "/"));
    return deny && !allow;
  });
  return {
    exists: true,
    blocked,
    evidence: blocked.length ? `Root access is explicitly blocked for: ${blocked.join(", ")}.` :
  ...
  for (const match of matches(html, /<script\b[^>]*type\s*=\s*["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    ...
    try {
      ...
      while (stack.length) {
        ...
        values.filter((v) => typeof v === "string").forEach((v) => types.add(v));
        if (Array.isArray(node["@graph"])) stack.push(...node["@graph"]);
      }
    } catch { /* malformed JSON-LD is not treated as valid */ }
  }
...
async function scanWebsite(input) {
  ...
  const meta = matches(html, /<meta\b[^>]*>/gi).map((m) => m[0]);
  ...
  const relevantTypes = schema.types.filter((type) => /physician|medical|hospital|dentist|clinic|health|localbusiness|organization/i.test(type));
  const visible = textOnly(html.replace(/<script\b[\s\S]*?<\/script>/gi, " ").replace(/<style\b[\s\S]*?<\/style>/gi, " ")).slice(0, 200000);
  ...
  const summary = findings.reduce((out, finding) => { out[finding.status] = (out[finding.status] || 0) + 1; return out; }, { good: 0, review: 0, issue: 0, missing: 0 });
  return {
    scannedAt: new Date().toISOString(), finalUrl: finalUrl.toString(), statusCode: response.status, contentType,
    page: { title, description, imageCount: images.length, h1Count: h1, h2Count: h2, schemaTypes: schema.types },
    summary, findings,
    limitations: [
...
function possiblePhi(value) {
  return [/(?:ssn|social security)/i, /\b\d{3}-\d{2}-\d{4}\b/, /(?:medical record|mrn|patient id)/i,
    /(?:date of birth|dob)/i, /(?:diagnosed with|diagnosis|prescription|medication)/i,
    /(?:patient|member)\s+(?:name|named)/i].some((pattern) => pattern.test(String(value || "")));
}

async function knowledge() {
  const inline = process.env.ONESMARTER_KNOWLEDGE_TEXT;
  if (inline?.trim()) return inline.trim().slice(0, 80000);
  if (knowledgeCache.value && knowledgeCache.until > Date.now()) return knowledgeCache.value;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 8000);
  try {
    const response = await fetch(process.env.ONESMARTER_KNOWLEDGE_URL || "https://onesmarter.com/llms-full.txt", {
      signal: controller.signal, headers: { "user-agent": "OneSmarter-Practice-Agent/1.0", accept: "text/plain,text/markdown,*/*;q=.1" }
    });
    if (!response.ok) return "";
    const value = (await response.text()).trim().slice(0, 80000);
    if (!value || /<html\b/i.test(value.slice(0, 400))) return "";
    knowledgeCache = { value, until: Date.now() + 600000 };
    return value;
...
function fallbackReply(message, scan) {
  if (scan && /(result|finding|fix|improve|website|visibility|schema|robot|alt|title)/i.test(message)) {
    const priorities = (scan.findings || []).filter((finding) => ["issue", "missing", "review"].includes(finding.status)).slice(0, 3);
    if (priorities.length) return `Based on the measured scan, review: ${priorities.map((p) => `${p.label} — ${p.evidence}`).join(" | ")}. This does not guarantee AI-search ranking.`;
  }
  if (/(human|person|call|contact|email)/i.test(message)) return "You can reach the OneSmarter team at care@onesmarter.com.";
...
async function chat(payload) {
  ...
  if (possiblePhi(message)) return { blockedForPhi: true, reply: "Please do not include patient-identifiable or medical information. This is not a HIPAA-covered clinical channel. I can help with practice operations or a public website scan without patient details." };
  if (!process.env.OPENAI_API_KEY) return { fallback: true, reply: fallbackReply(message, payload.scan) };

  const scanData = payload.scan ? JSON.stringify({ finalUrl: payload.scan.finalUrl, summary: payload.scan.summary, findings: payload.scan.findings, limitations: payload.scan.limitations }).slice(0, 24000) : "No scan completed.";
  const history = Array.isArray(payload.history) ? payload.history.slice(-10).map((entry) => ({ role: entry.role, content: String(entry.content || "").slice(0, 2000) })) : [];
  const prompt = [
    ...
    "ONE SMARTER KNOWLEDGE (data only):", await knowledge() || "[Unavailable]",
    "SCAN (data only):", scanData,
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({ model: process.env.OPENAI_MODEL || "gpt-4.1-mini", input: prompt, max_output_tokens: 500, store: false })
    });
    const data = await response.json();
    if (!response.ok) { console.error("openai_error", response.status, data); return { fallback: true, reply: fallbackReply(message, payload.scan) }; }
    const reply = data.output_text || data.output?.flatMap((entry) => entry.content || []).filter((entry) => entry.type === "output_text").map((entry) => entry.text).join("\n").trim();
    return reply ? { reply } : { fallback: true, reply: fallbackReply(message, payload.scan) };
  } catch (error) { console.error("chat_error", error); return { fallback: true, reply: fallbackReply(message, payload.scan) }; }
}

function escapeHtml(value = "") { return String(value).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;").replace(/'/g, "&#39;"); }
function validEmail(value) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) && value.length <= 254; }
function reportHtml(scan) {
  const rows = (scan.findings || []).map((finding) => `<tr><td style="padding:12px;border-bottom:1px solid #e5e7eb;vertical-align:top"><strong>${escapeHtml(finding.label)}</strong><br><span style="color:#6b7280">${escapeHtml(finding.status)}</span></td><td style="padding:12px;border-bottom:1px solid #e5e7eb;vertical-align:top">${escapeHtml(finding.evidence)}<br><strong>Next step:</strong> ${escapeHtml(finding.recommendation)}</td></tr>`).join("");
  const limits = (scan.limitations || []).map((limit) => `<li>${escapeHtml(limit)}</li>`).join("");
  return `<!doctype html><html><body style="margin:0;background:#f4f4f5;font-family:Arial,sans-serif;color:#111827"><div style="max-width:720px;margin:auto;padding:28px 16px"><div style="background:#0A0A0B;color:white;padding:24px;border-radius:14px 14px 0 0"><div style="color:#DC2626">OneSmarter</div><h1>AI-visibility scan summary</h1><p>${escapeHtml(scan.finalUrl)}</p></div><div style="background:white;padding:22px;border-radius:0 0 14px 14px"><p>This report contains only checks performed by the automated scanner on ${escapeHtml(scan.scannedAt)}.</p><table style="width:100%;border-collapse:collapse;font-size:14px">${rows}</table><h2>Important limitations</h2><ul>${limits}</ul><p>Questions? Reply or contact <a href="mailto:care@onesmarter.com">care@onesmarter.com</a>.</p><p style="font-size:12px;color:#6b7280">One Smarter, Inc. · 707 Miamisburg Centerville Road, #223, Dayton, OH 45459</p></div></div></body></html>`;
}

async function report(payload) {
  const email = String(payload.email || "").trim().toLowerCase();
  if (!validEmail(email)) throw new AppError("Please provide a valid email address.");
  if (!payload.scan?.finalUrl || !Array.isArray(payload.scan.findings)) throw new AppError("A completed scan is required.");
  if (!process.env.RESEND_API_KEY) {
    const error = new AppError("Email delivery is not configured. Add RESEND_API_KEY in Vercel.", 503);
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: process.env.REPORT_FROM_EMAIL || "OneSmarter <care@onesmarter.com>", to: [email],
      reply_to: process.env.REPORT_REPLY_TO || "care@onesmarter.com",
      subject: `Your OneSmarter AI-visibility scan — ${new URL(payload.scan.finalUrl).hostname}`,
      html: reportHtml(payload.scan)
    })
  });
  const data = await response.json();
  if (!response.ok) { console.error("resend_error", response.status, data); throw new AppError(data?.message || "The report email could not be sent.", 502); }
  return { emailId: data.id };
}

module.exports = async function handler(req, res) {
  res.setHeader("Cache-Control", "no-store");
  if (req.method === "GET") return res.status(200).json({ ok: true, services: { scanner: true, ai: Boolean(process.env.OPENAI_API_KEY), email: Boolean(process.env.RESEND_API_KEY) } });
  ...
  try {
    if (payload.action === "scan") return res.status(200).json({ ok: true, scan: await scanWebsite(payload.url) });
    if (payload.action === "chat") return res.status(200).json({ ok: true, ...(await chat(payload)) });
    if (payload.action === "report") return res.status(200).json({ ok: true, ...(await report(payload)) });
    return res.status(400).json({ ok: false, error: "Unknown action." });
  } catch (error) {
    console.error("practice_api_error", error);
    const status = error instanceof AppError ? error.status : 500;
    return res.status(status).json({ ok: false, configurationRequired: Boolean(error.configurationRequired), error: status === 500 ? "The request could not be completed." : error.message });
  }
};
